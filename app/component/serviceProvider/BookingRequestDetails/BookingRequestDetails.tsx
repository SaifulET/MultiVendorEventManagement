...
import {
  ...
  fetchServiceProviderBookingDetails,
  formatBookingDate,
  formatBookingTime,
  formatCurrencyMinor,
  getBookingStatus,
...
const buildBookingData = (booking: ServiceProviderBooking): BookingData => {
  ...
  return {
    client: {
      avatar: booking.customer?.profileImage || undefined,
    ...
    location: {
      ...
      venue: getServiceName(booking),
    },
    pricing: {
      currency: booking.pricing?.currency ?? 'USD',
      servicePrice: booking.pricing?.unitAmount ?? 0,
      tax: booking.pricing?.taxAmount ?? 0,
      taxRate,
      total: booking.pricing?.totalAmount ?? 0,
    },
    requestedDate: formatBookingDate(booking.bookingDate),
    service: {
      description: getServiceDescription(booking),
      title: getServiceName(booking),
    },
    status: getBookingStatus(booking),
  };
};

const formatAmountForDisplay = (amount: number, currency: string) => formatCurrencyMinor(amount, currency);
...
export default function BookingRequestDetails() {
  ...
  const [booking, setBooking] = useState<ServiceProviderBooking | null>(null);
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      ...
      try {
        ...
        const response = await fetchServiceProviderBookingDetails(bookingId);
  ...
  const googleMapsEmbedUrl = `https://www.google.com/maps/embed/v1/place?q=${encodeURIComponent(bookingData.location.address)}&key=YOUR_API_KEY&zoom=15`;
                ...
                <div className="flex items-start gap-4 mb-4">
                  <div className="w-12 h-12 rounded-full overflow-hidden bg-gray-200 flex items-center justify-center text-gray-600 font-semibold text-lg">
                    {bookingData.client.avatar ? (
                      ...
                      bookingData.client.name.charAt(0)
                    )}
                  </div>
            ...
            <div className="bg-white rounded-lg border border-[#E5E7EB] p-6">
              ...
              <div className="flex items-start gap-3">
                <div className="w-10 h-10 rounded-lg bg-blue-100 flex items-center justify-center flex-shrink-0">
                  <span className="text-blue-600 text-lg">S</span>
                </div>
                <div>
              ...
              <div className="space-y-3 mb-6">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-semibold text-gray-900">
                    {formatAmountForDisplay(bookingData.pricing.servicePrice, bookingData.pricing.currency)}
                  </span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-600">TAX ({bookingData.pricing.taxRate}%)</span>
                  <span className="font-semibold text-gray-900">
                    {formatAmountForDisplay(bookingData.pricing.tax, bookingData.pricing.currency)}
                  </span>
                </div>
                <div className="border-t border-gray-200 pt-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xl font-bold text-gray-900">
                      {formatAmountForDisplay(bookingData.pricing.total, bookingData.pricing.currency)}
                    </span>
                  </div>
                </div>
              </div>
