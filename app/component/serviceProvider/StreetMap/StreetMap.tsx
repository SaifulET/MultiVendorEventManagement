"use client";

import { MapContainer, TileLayer, Marker, useMapEvents } from "react-leaflet";
import "leaflet/dist/leaflet.css";
import { useState } from "react";
import L from "leaflet";

/* ---------------------------------------------
   Fix Leaflet default marker icon
---------------------------------------------- */

interface LeafletIconDefault extends L.Icon.Default {
  _getIconUrl?: () => string;
}

delete (L.Icon.Default.prototype as LeafletIconDefault)._getIconUrl;

L.Icon.Default.mergeOptions({
  iconRetinaUrl:
    "https://unpkg.com/leaflet@1.9.4/dist/images/marker-icon-2x.png",
  iconUrl:
    "https://unpkg.com/leaflet@1.9.4/dist/images/marker-icon.png",
  shadowUrl:
    "https://unpkg.com/leaflet@1.9.4/dist/images/marker-shadow.png",
});

/* ---------------------------------------------
   Types
---------------------------------------------- */

export type Coordinates = {
  lat: number;
  lng: number;
};

type MapPickerProps = {
  onSelect: (coords: Coordinates) => void;
  initialPosition?: Coordinates; // This will show the previous location
};

/* ---------------------------------------------
   Marker Component - SIMPLIFIED VERSION
---------------------------------------------- */

function LocationMarker({ onSelect, initialPosition }: MapPickerProps) {
  // Start with initialPosition if it exists, otherwise null
  const [position, setPosition] = useState<Coordinates | null>(initialPosition || null);

  useMapEvents({
    click(event) {
      const coords: Coordinates = {
        lat: event.latlng.lat,
        lng: event.latlng.lng,
      };

      setPosition(coords);
      onSelect(coords);
    },
  });

  // Show marker if we have a position (either initial or newly selected)
  return position ? <Marker position={[position.lat, position.lng]} /> : null;
}

/* ---------------------------------------------
   Main MapPicker Component
---------------------------------------------- */

export default function MapPicker({ onSelect, initialPosition }: MapPickerProps) {
  // Default to Evanston if no initial position
  const defaultCenter = { lat: 42.0451, lng: -87.6877 };
  
  // Use initialPosition as center if it exists, otherwise use default
  const center = initialPosition || defaultCenter;
  
  return (
    <MapContainer
      center={center}
      zoom={13}
      style={{ height: "400px", width: "100%" }}
      scrollWheelZoom
    >
      <TileLayer
        attribution="&copy; OpenStreetMap contributors"
        url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
      />

      <LocationMarker onSelect={onSelect} initialPosition={initialPosition} />
    </MapContainer>
  );
}