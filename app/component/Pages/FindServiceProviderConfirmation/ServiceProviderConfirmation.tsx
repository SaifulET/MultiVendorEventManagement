...
const ServiceProviderConfirmation: React.FC = () => {
  ...
  const serviceName = bookingContext?.target?.information?.serviceName?.trim() || 'Service';
  ...
  const priceDisplay = formatCurrencyAmount(
    bookingContext?.target?.pricing?.amount,
  ...
  useEffect(() => {
    ...
    if (!serviceId) {
      setContextError('Service not found.');
  ...
  const handleConfirmBooking = async () => {
    ...
    if (!token || !user) {
      setDialog({
        ...
        message: 'Please sign in with a customer account before booking this service provider.',
      });
      return;
    }
    ...
    if (user.role !== 'customer') {
      setDialog({
        ...
        message: 'Only users with the customer role can book service providers.',
      });
      return;
    }
    ...
    if (!serviceId) {
      setDialog({
        ...
        message: 'We could not find the service you are trying to book.',
      });
      return;
    }
    ...
    try {
      await Promise.all(
        selectedDateKeys.map((dateKey) =>
          api.post(`/api/v1/bookings/services/${serviceId}`, {
            bookingDate: dateKey,
            ...
            <div className="bg-white rounded-lg border border-[#E5E7EB] p-6">
              <div className='flex items-center justify-between'>
                ...
                <div className="w-[198px] flex flex-col justify-center items-center gap-2">
                  ...
                  <p className='font-inter font-normal text-[10px] leading-[1] tracking-normal text-center'>If you keep the Notify Me button active, you will receive an email as soon as this service becomes available.</p>
                </div>
              </div>
            ...
            <div className="bg-white rounded-lg border border-[#E5E7EB] p-6">
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">By {providerName}</p>
                </div>
              </div>
              ...
              <p className="mb-4 text-sm text-gray-600">{serviceDescription}</p>
